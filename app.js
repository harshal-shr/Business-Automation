/* =========================================================
   botmart — AI Systems Marketplace (front-end demo)
   Vanilla JS + Bootstrap 5 + localStorage
   ========================================================= */
...
const SEED_SYSTEMS = [
  { id:'t2', title:'t2', desc:'nsdd fd nd a', by:'Testing Desktop', status:'BOOKED',
    tags:['OpenAI','Python'], avatar:dice('TestingDesktop') },
  { id:'t1', title:'t1', desc:'olhsd if iof diosd', by:'Testing Desktop', status:'BOOKED',
    tags:['LangChain','Database'], avatar:dice('TestingDesktop2') },
];
const SEED_NEEDS = [
  { id:'b2', title:'b2', desc:'jn msd id fddoj s', by:'bus', duration:'3 weeks', status:'BOOKED', avatar:dice('b2','shapes') },
  { id:'b1', title:'b1', desc:'jndd df insd sdinds', by:'bus', duration:'2 weeks', status:'BOOKED', avatar:dice('b1','shapes') },
];
...
/* ---------- routing ---------- */
const PAGES = ['auth','feed','needs','creator','inbox','profile'];

function go(page){
  if(!user && page !== 'auth') page = 'auth';
  ...
  if(page === 'feed')   renderSystems();
  if(page === 'needs')  renderNeeds();
  if(page === 'creator')renderCreator();
  if(page === 'profile')renderProfile();
  const c = $('#navMenu'); if(c && c.classList.contains('show')) bootstrap.Collapse.getInstance(c)?.hide();
  window.scrollTo(0, 0);
}
...
function renderSystems(){
  ...
  $('#systemsGrid').innerHTML = list.length
    ...
    : `<div class="col-12"><div class="empty-state">No systems match your search attributes.</div></div>`;
}
...
function needCard(n){
  return `
  <div class="col-12">
    <div class="card bm-card need-card">
      <div class="card-body">
        <div class="d-flex align-items-center gap-2">
...
function renderNeeds(){
  ...
  $('#needsGrid').innerHTML = list.length
    ...
    : `<div class="col-12"><div class="empty-state">No business pipelines indexed for this filter.</div></div>`;
}
